...
class BookRepository:
    ...
    def load_books(self):
        '''
        Function to load books from data.json into the __books dictionary
        '''
    ...
    def get_books(self) -> Iterable[Book]:
        '''
        Method that returns an iterator object with book data
        '''
        return self.__books.values()

    def save_books(self):
        '''
        Method to save the books stored in __books to data.json
        '''
    ...
    def add_book(self, title: str, author: str, year: int):
        ...
        new_id: int = max(self.__books.keys()) + \
            1 if len(self.__books) > 0 else 0
